...
**`open_door`** - Grip the fridge handle and pull the door open along its hinge arc, then release and retract, leaving the door open. The door is a static obstacle (`fridge-door`) whose root frame origin sits on the hinge; `open_door` sweeps the door angle in software (`door_open_angle_degs`, default 90°, in `door_pivot_degrees_per_step` increments, default 10°), re-placing the door obstacle at each step so the grasp frame (`door_grasp_frame_name`, default `fridge-handle-ball`) and the door panel track the real swing and collision-checking stays honest. The gripper aims at the grasp frame's **center** with the approach pose's orientation — the grasp isn't a separately-authored pose — and contact between the gripper and that frame is allowed during the pull. Requires the `door-approach` and `door-retract` poses to be authored on the claws pose switch (the approach pose's `component_name` should be the gripper's grasp point, since that's what gets aimed at the ball center). Gated like `run_cup_flow` (one sequence at a time) and honors `cancel`; the frame system is rebuilt on exit so the door mutation never leaks.
