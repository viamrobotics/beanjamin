...
| `camera_observe_pose_switcher_name`   | string | When `dynamic_cup_pickup` is enabled | A dedicated `viam:beanjamin:multi-poses-execution-switch` holding the camera observation vantages. Every pose on the switch is visited and vision is run at each; their detections are merged in world frame (near-duplicates within 40 mm collapsed) before pickup ranking and shelf-tile selection. The switch must include a pose named `cup_observe`, used as the home/recovery pose between grab attempts. All its poses must move the camera frame `cam` (set the switch's `component_name` to `cam`). Add extra vantages to mitigate single-frame YOLO misses and shelf-cup occlusion. |
...
| `cup_photos_per_vantage`              | int    | No       | How many vision frames to capture at each observation pose. Every detection from every frame feeds the cross-vantage merge, so more photos average out per-frame centroid noise. Default 1. |
...
When `dynamic_cup_pickup` is enabled, the dedicated camera-observe switch must hold one or more observation poses, all moving the camera frame `cam` (the switch's `component_name`). Every pose is visited and vision is run at each; detections are merged across vantages. The switch must include a pose named `cup_observe`.
