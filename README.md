...
**`execute_action: close_door`** - The reverse of `open_door`: grip the open door's handle and push the panel back shut, then release and retract. Same grasp derivation, fixed-orientation tracking, per-step obstacle re-placement, cancellation, and frame-system reset — only the sweep endpoints differ (`door_open_angle_degs` → 0° instead of 0° → `door_open_angle_degs`). One caveat: the frame system always rebuilds with the door at its authored **closed** transform, so `close_door` begins by placing the door obstacle at `door_open_angle_degs` before resolving the grasp. It therefore assumes the door really is open at that angle — true when it follows `open_door`, but running it against an already-closed door will aim the gripper at where the handle *would* be if the door were open.
