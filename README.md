...
The pause is only released if there is one — `proceed` on a running queue rebuilds the world and reports `resumed: false`. Releasing the pause *is* clearing the queue's paused flag, done by `proceed` itself: any cancel pauses the queue, including one that interrupts an `execute_action` or a keepalive purge with no order in flight, and such a pause has no order-queue goroutine waiting to hear about it. Two `proceed`s racing therefore resume once, and the second reports `resumed: false`. `proceed` refuses outright while a sequence is still running (including one that is unwinding from a cancel): the frame system cannot be swapped under a goroutine that is planning with it. The same one-at-a-time rule covers every command that moves the arm — `execute_action`, `run_cup_flow`, `rewind`, `reset_world`, a keep-alive purge — each refused with "a sequence is already running" while another holds the arm. Queued orders are the exception: an order that comes up while the arm is busy, or while the queue is paused, stays pending (still listed by `get_queue`, still removable by `cancel_order`) and starts as soon as the arm is free and the queue unpaused. The `action` command (`open_gripper` / `close_gripper`) is a manual override and is never refused.
...
Because a purge is the one arm motion nobody requested, it announces itself through `speech_service_name` and waits 5 seconds before moving, regardless of `conversational` — it's a safety notice rather than status narration. The arm returns to `home` when the purge finishes. A purge never competes with an order: it is skipped while one is queued or brewing, and an order placed once a purge is under way stays queued until the arm is back, then starts as normal.
