...
| `button_press_hold_sec`    | float  | No       | How long the claw dwells on a brew button so the momentary switch registers, in seconds (default: 0.5; `0` is a valid explicit value, meaning no dwell). Raise if presses are intermittent. Ignored unless `has_separate_brew_buttons` is set. |
