...
| `cup_vision_service_name`             | string | Yes      | Name of a `rdk:service:vision` segmenter that returns cup detections via `GetDetections3D`, each a transform tree parented to `world`. Cup pickup is always vision-guided — the arm detects the empty cup rather than grabbing from a fixed pose. |
...
| `glass_vision_service_name`           | string | When `can_serve_iced` is enabled | Name of a `rdk:service:vision` segmenter that returns glass detections via `GetDetections3D`. Glass pickup mirrors cup pickup but with its own vision service and observe poses (tuned for the taller iced-coffee glass). |
...
| `milk_vision_service_name`            | string | When `can_serve_iced_latte` is enabled | Name of a `rdk:service:vision` segmenter that returns milk-bottle detections via `GetDetections3D`. Milk pickup mirrors cup and glass pickup with its own vision service and observe poses. |
...
**No-spill carry.** Every **free traverse of a filled container** is planned with a **20° orientation constraint** (`noSpillOrientationToleranceDegs` in `coffee/motion.go`) instead of free-planning straight to the goal. RDK checks the constraint along the whole path as a tube of that width around the **direct slerp** from the carry's start orientation to its goal, so one start-to-goal segment holds the container near level for the entire traverse — no intermediate waypoints are needed. Both endpoints are upright container poses, so the slerp itself stays level and 20° leaves the drink well inside a full cup's static spill angle. The path itself is unconstrained: the planner is free to route around the machine however it likes, subject only to collision avoidance and the orientation tube. This covers the serving-area placement of a **filled** container (the hot cup and the iced glass), carrying the ice-filled glass to the staging area, and carrying the espresso cup to the pour position. The espresso cup's own trip to the shelf happens *after* it has been poured over the ice, so it is empty by then and free-plans straight to the slot — there is nothing left in it to slosh. Manual `place_held` assumes filled. The milk bottle is **not** carried level — it is tall and grasped high, so whether the level carry is plannable for it is an open question; its carries free-plan for now. The goal commands the **held-item (container) frame** rather than the gripper — the goal pose is converted onto it — because the orientation bound is measured in the commanded frame's own axes, and the held-item frame's **+Z is the container's vertical axis** (see **Held-item geometry** below). Tune the tolerance on hardware.
