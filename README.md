...
| `door_plan_attempts`       | int    | No       | How many independent approach branches a door sweep plans before committing to the cheapest by total joint travel. Defaults to 3; `1` commits to the first that plans. The approach goal is a Cartesian standoff, so the planner may reach it on any IK branch and scores that choice only on getting there — a branch that barely reaches drags the sweep through large joint excursions and shears the handle out of the jaws. Each attempt costs a full approach + grasp + one plan per θ waypoint before the arm moves, so this trades latency for posture. |
