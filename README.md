...
| `no_spill_carry`                      | bool   | No       | When `true`, every **free traverse of a filled container** is carried along a path broken into waypoints (one every 100 mm, measured along that path) instead of free-planning straight to the goal. The path **sweeps around the world Z axis** — the arm's own base axis — rather than following the chord between the endpoints: radius, azimuth and height are interpolated separately, so the container stays between the two endpoint radii for the whole traverse. A straight chord between two points at similar reach cuts inward, toward the base and through the machine the arm is standing at (the glass placement's chord passes 115 mm nearer the axis than either of its own endpoints); the sweep costs a few percent of path length to avoid that. When both endpoints share an azimuth the sweep *is* the straight line, and an endpoint on the Z axis — where azimuth is meaningless — falls back to it. This covers the serving-area placement of a **filled** container (the hot cup and the iced glass), carrying the ice-filled glass to the staging area, and carrying the espresso cup to the pour position. The espresso cup's own trip to the shelf happens *after* it has been poured over the ice, so it is empty by then and free-plans straight to the slot — there is nothing left in it to slosh. Manual `place_held` assumes filled. The milk bottle is **not** carried level — it is tall and grasped high, so whether the level-carry pose cloud is plannable for it is an open question; its carries free-plan for now. Each waypoint commands the **held-item (container) frame** — the start and goal poses are converted onto it — interpolating the pose from the container's **upright** start to the goal, with a goal pose cloud that keeps the **orientation** tight (so the drink stays close to level) while opening up **translational** slack generously (position can't tip the drink, so the extra reach is free). The **final** waypoint — the true target — is pinned exactly with no pose cloud, so the following linear descent or pour pivot starts from the intended pose. The waypoints are planned as one multi-goal trajectory. The pose-cloud leeways are constants in `coffee/motion.go` (`noSpillGoalCloud`). They are measured in the commanded frame's own axes, and the held-item frame's **+Z is the container's vertical axis** (see **Held-item geometry** below), so `ox`/`oy` bound the drink's tilt off vertical while the wide `theta` is a harmless spin about the container's own axis. Tune them on hardware. The pose cloud shapes only the **goals**, so the trajectory is unconstrained between them; the carry therefore also carries a **50° path orientation constraint** (`noSpillOrientationToleranceDegs`), checked along the whole path, which caps how far the container may swing off a segment's own interpolation between two waypoints. Default `false` (those moves free-plan straight to the goal pose). |
...
**`send_daily_summary`** - Post a Slack digest of the orders recorded so far today. Normally fired on a schedule by viam-server's job manager (see "Daily order summary in Slack" below); calling it by hand is how you test the digest off-schedule. Requires both `slack_notifier_name` and `order_sensor_name`.

```json
{"send_daily_summary": {"timezone": "America/New_York"}}
```

`timezone` is an IANA name that decides where the business day starts — the digest covers local midnight up to the moment it runs. It defaults to the host's timezone when omitted (`{"send_daily_summary": true}` is a valid hand-fired call). Returns `{"sent": true, "orders": N}`.
...
"jobs": [
  {
    ...
    "schedule": "CRON_TZ=America/New_York 30 17 * * 1-5",
    "resource": "coffee",
    "method": "DoCommand",
...
> ⚠️ **The `CRON_TZ=` prefix is not optional.** The job manager builds its scheduler without a location, so a bare `"30 17 * * 1-5"` fires at 17:30 in the *host's* timezone — four hours off from New York, and silently so. Keep the prefix and the command's `timezone` set to the same zone: the first decides when the digest runs, the second decides where the business day starts. The digest prints the window it actually covered in its footer (`12:00 AM – 5:30 PM EDT`), so a mismatch shows up in the message rather than going unnoticed.

`1-5` is Monday–Friday; drop it for every day. `method` must be `DoCommand` — the job manager has a fast path for it that calls the service directly instead of going through gRPC reflection.

Two consequences worth knowing. The window ends when the digest runs, so **orders placed after 17:30 never appear in any digest** — that is the price of reporting the day while it is still the day. And a job that fails is only a log line plus an entry in the job's history; nothing is posted to Slack, so a broken digest looks the same as a channel nobody used. The "No orders today" heartbeat is the cheap check against that.
