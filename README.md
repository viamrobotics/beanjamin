...
| `cup_observe_pose_switcher_name`      | string | When `dynamic_cup_pickup` is enabled | A dedicated `viam:beanjamin:multi-poses-execution-switch` holding the cup-observation vantages. Every pose on the switch is visited and vision is run at each; their detections are merged in world frame (near-duplicates within 40 mm collapsed) before pickup ranking and shelf-tile selection. The switch must include a pose named `cup_observe`, used as the home/recovery pose between grab attempts. All its poses must target the `coffee-claws-middle` frame. Add extra vantages to mitigate single-frame YOLO misses and shelf-cup occlusion. |
...
**Dynamic cup pickup — required poses on the cup-observe pose switcher (`cup_observe_pose_switcher_name`):**

When `dynamic_cup_pickup` is enabled, the dedicated cup-observe switch must hold one or more observation poses, all targeting the `coffee-claws-middle` frame. Every pose is visited and vision is run at each; detections are merged across vantages. The switch must include a pose named `cup_observe`.
