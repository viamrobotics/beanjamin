...
| `portafilter_shake_sec`    | float  | No       | Duration in seconds of a small circular shake at the `coffee_shake` pose during `unlock_portafilter`, to dislodge a stuck puck. When set, requires a `coffee_shake` pose in the filter pose switcher. Defaults to 0, which skips both the shake and the travel to that pose: the arm withdraws straight from `coffee_in` to `coffee_approach`. |
