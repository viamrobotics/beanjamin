...
**Pause after a fault.** An order that fails on its own — a genuine fault, not a `cancel` or `reset_world` — pauses the queue exactly as `cancel` does whenever it leaves the machine mid-cycle: the portafilter locked in the group head or holding grounds, the filter frame locked to world, an item in the gripper, a glass staged, or the fridge door modeled open. The next order would otherwise start from that unknown physical state, grinding a second dose onto used grounds or planning against a stale world. An error log names the state left behind, and `get_queue` reports `is_paused: true`; recover the same way as after a cancel (`rewind`, shut the fridge by hand if it is open, then `proceed`). A fault that left nothing behind — cup pickup finding no cup before anything was grabbed, say — keeps the queue running. A failed manually-stepped `execute_action` never pauses the queue.
