...
**Usage sensors.** The optional `grinder_sensor_name`, `decaf_grinder_sensor_name`, `water_sensor_name`, `cleaner_sensor_name`, and `consecutive_orders_sensor_name` fields each point at a sensor resource that tracks a single counter through the brew lifecycle. Setting a field automatically registers the named sensor as a dependency of the coffee service, so no manual `depends_on` entry is required. Each sensor must support both the `Readings` API and a `DoCommand({"set": {<field>: <value>}})` that overwrites its counter. The coffee service updates them with a best-effort read-modify-write: it reads the current value via `Readings`, computes the new value, and writes it back via `DoCommand`. Counters increment only after their step completes successfully — `grinds` per grind, `usage` per brew (1 regular / 1.5 lungo), and `cleanings` per cleaning — so a brew that fails partway leaves the consumables it actually used counted and does not roll them back. The `successful_consecutive_orders` streak increments on each successful order and resets to 0 on any failed **or** operator-cancelled order. A missing counter field is treated as 0 (so the first update lands a fresh count). All updates are best-effort: a read/write failure logs a warning and never fails the brew. Any sensor left unconfigured is simply skipped.
...
| `grinder_sensor_name`      | string | No       | Name of a sensor whose `grinds` counter is incremented by 1 after each regular (non-decaf) grind. See "Usage sensors" below. |
| `decaf_grinder_sensor_name` | string | No      | Name of a sensor whose `grinds` counter is incremented by 1 after each decaf grind. |
| `water_sensor_name`        | string | No       | Name of a sensor whose `usage` counter is incremented after each brew: by `1` for regular drinks (espresso/decaf) and `1.5` for lungo drinks (lungo/decaf_lungo). |
| `cleaner_sensor_name`      | string | No       | Name of a sensor whose `cleanings` counter is incremented by 1 after each cleaning cycle. |
| `consecutive_orders_sensor_name` | string | No | Name of a sensor whose `successful_consecutive_orders` counter is incremented by 1 after each successful order and reset to 0 after any failed or cancelled order. |
