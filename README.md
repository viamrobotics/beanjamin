...
| `door_plan_attempts`       | int    | No       | How many approach branches a door sweep tries before giving up. Defaults to 3. The approach goal is a Cartesian standoff, so the planner may reach it on any IK branch and scores that choice only on getting there; under the sweep's linear constraint a branch that cannot carry the swing fails to plan, and another attempt gets a different branch. Each attempt costs a full approach + grasp + one plan per θ waypoint before the arm moves. |
| `door_sweep_linear_constraint` | object | No   | `{ "line_tolerance_mm", "orientation_tolerance_degs" }` holding the gripper to a straight line between consecutive θ waypoints, so it cannot bow off the hinge arc between them. Defaults to 5 mm / 5°. Deliberately looser than the 1 mm / 2° used for grasp descents: the constraint holds the tool to the **chord** while the gripped handle travels the **arc**, and the two diverge mid-segment by `R·(1−cos(step/2))` — about 1.9 mm at a 557 mm handle radius and the default step. A tighter tolerance fights the door. Halving `door_pivot_degrees_per_step` quarters that divergence, so the two move together. Set both tolerances negative to sweep unconstrained. |
