...
func rowProfile(path string, r roi) ([]float64, error) {
	...
	rows, err := coffee.RowBrightness(img, r.x0, r.x1, r.y0, r.y1)
	if err != nil {
...
// contrastStep is the service's own step measurement. Calibrating against a
// second copy of it would tune values for a measurement that does not ship.
func contrastStep(rows []float64, w int) (step float64, idx int) {
	return coffee.ContrastStep(rows, w)
}
...
func reportLevels(dir string, frames []rawFrame, r roi, csvPath string) error {
	...
	if lowestNonZero >= 0 {
		...
		if highestZero >= 0 {
			fmt.Printf("                   (%.0f%% read zero — nothing below %.0f%% registers at this pose)\n",
				highestZero*100, lowestNonZero*100)
		}
	}
...
func reportContrast(dir string, frames []rawFrame, r roi) error {
	...
	fmt.Printf("  full glass          = %.0f px   (R² %.4f — the fit that guarded this\n", best.slope, best.r2)
	fmt.Printf("                       window choice, not a value to configure)\n")
