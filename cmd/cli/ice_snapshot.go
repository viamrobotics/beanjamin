...
func runIceSnapshot(args []string) error {
	...
	if *glassHeight < 0 {
		return fmt.Errorf("--glass-height must not be negative")
	}
...
// summarizeStats renders a mean, plus a standard deviation when there is more than
// one sample.
func summarizeStats(vals []float64, format string) string {
	if len(vals) == 0 {
		return "n/a"
	}
	if len(vals) == 1 {
		return fmt.Sprintf(format, mean(vals))
	}
	return fmt.Sprintf(format+" ± "+format, mean(vals), stddev(vals))
}
