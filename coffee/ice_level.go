...
func iceBandFromConfig(cfg *Config) iceBand {
	...
	return iceBand{
		...
		y0:          orDefault(cfg.IceStopRowPx, defaultIceStopRowPx) - window,
		y1:          orDefault(cfg.IceROIY1, defaultIceROIY1),
		window:      window,
...
// loudly. At ice_min_contrast 0 every frame reports a surface; a transposed x
// pair scans nothing and never sees one. Both look like working config from the
// outside.
//
...
func validateIceVision(cfg *Config, path string) error {
	...
	if b.x0 >= b.x1 {
		return fmt.Errorf("%s: ice_roi_x0 (%d) must be left of ice_roi_x1 (%d) — a band with no width scans nothing, never sees a surface, and every dispense runs to ice_dispense_max_sec", path, b.x0, b.x1)
	}
	if b.y0 < 0 {
		return fmt.Errorf("%s: ice_stop_row_px (%d) must be at least ice_contrast_window (%d) — the band opens a full window above the stop row", path, b.y0+b.window, b.window)
	}
	...
	if b.y0+2*b.window >= b.y1 {
		return fmt.Errorf("%s: ice_roi_y1 (%d) must be more than one contrast window (%d) below ice_stop_row_px (%d) — otherwise the ice surface can never be found at the stop row", path, b.y1, b.window, b.y0+b.window)
	}
	if b.minContrast <= 0 {
		return fmt.Errorf("%s: ice_min_contrast must be greater than 0 — at 0 every frame reports an ice surface", path)
	}
	return nil
}
...
func iceSurfaceRow(img image.Image, b iceBand) (iceReading, error) {
	...
	if idx < 0 || step < b.minContrast {
		return iceReading{}, nil
	}
	return iceReading{row: b.y0 + idx, step: step, found: true}, nil
}
...
// rowBrightness returns the mean brightness of each row in the band, top to
// bottom.
func rowBrightness(img image.Image, b iceBand) ([]float64, error) {
	rows, err := RowBrightness(img, b.x0, b.x1, b.y0, b.y1)
	if err != nil {
		// Every row here is a raw pixel, so a camera reconfigured to a different
		// frame size invalidates all of them silently and in an unknown direction.
		return nil, fmt.Errorf("%w — the ice_roi_* and ice_stop_row_px values are pixels at the resolution they were measured at", err)
	}
	return rows, nil
}

// RowBrightness returns the mean brightness of each row of the x0..x1 strip,
// from y0 down to y1.
//
// Exported for cmd/cli's ice calibration commands: the tool that picks
// ice_min_contrast and ice_contrast_window has to measure exactly what the
// service measures, or it calibrates values for a different measurement.
func RowBrightness(img image.Image, x0, x1, y0, y1 int) ([]float64, error) {
	if x0 >= x1 || y0 >= y1 || y0 < 0 {
		return nil, fmt.Errorf("empty ice band: x %d..%d, y %d..%d", x0, x1, y0, y1)
	}
	bounds := img.Bounds()
	if x1 > bounds.Max.X || y1 > bounds.Max.Y {
		return nil, fmt.Errorf("ice band x %d..%d y %d..%d is outside the %dx%d frame",
			x0, x1, y0, y1, bounds.Max.X, bounds.Max.Y)
	}
	rows := make([]float64, 0, y1-y0)
	width := float64(x1 - x0)
	for y := y0; y < y1; y++ {
		var sum float64
		for x := x0; x < x1; x++ {
...
//
// Exported as ContrastStep for the same reason as RowBrightness.
...
// ContrastStep is contrastStep, exported for cmd/cli's ice calibration commands.
func ContrastStep(rows []float64, w int) (step float64, idx int) {
	return contrastStep(rows, w)
}

// measureIceSurface grabs a frame and measures it.
func (s *beanjaminCoffee) measureIceSurface(ctx context.Context) (iceReading, error) {
	img, err := s.iceFrame(ctx)
	if err != nil {
		return iceReading{}, err
	}
	return iceSurfaceRow(img, s.iceBand())
}
...
func (s *beanjaminCoffee) checkIceLevel(ctx, _ context.Context) error {
	...
	b := s.iceBand()
	surface, err := iceSurfaceRow(img, b)
	if err != nil {
		return fmt.Errorf("check_ice_level: %w", err)
	}
	bounds := img.Bounds()
	...
	logger.Infof("check_ice_level: strongest step above the band at row %d (step %.0f) — on an empty glass that is the glass rim, and how the seating is checked; the fixtures ice_stop_row_px was chosen against sit at 277 and 379",
		rim.row, rim.step)
	return nil
}
